...
class Offer extends Component {
    ...
    return (
      <ul className='flex flex-row overflow-hidden'>
        {this.props.technologies.map(technology => {
          return (
            <li key={technology.id} className='px-2 mx-2 text-gray-600 border-2 rounded-full uppercase border-gray-400'>
              {technology.name}
            </li>
          );
        })}
  ...
  render() {
    const { description, position, location } = this.props; //add salary and techstack
    return (
      <div className='flex flex-row text-black shadow-2xl border-2 border-darken-1 rounded-lg'>
        <img
        ...
        <div className='flex flex-col justify-around w-full'>
          ...
          <div className='flex flex-row justify-between'>
            <div className='flex p-2 mx-2'>
              <img src={require('../img/building_icon.png')} className='px-2' alt='building icon' />
